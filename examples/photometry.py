...
if __name__ == '__main__':
    ...
    colors = {f:plt.rcParams['axes.prop_cycle'].by_key()['color'][ind]
    ...
    for z in zs:
        ...
        seds[z] = galaxy.get_pacman_spectra(grid, fesc=0.5, fesc_LyA=0.5,
                                            tauV=0.1, sed_object=True)
    ...
    for ind, z in enumerate(zs):
        ...
        for f in filters:
            ...
            # Plot the photometry
            ax.scatter(f.pivwv(), phot, s=50, color=colors[f.filter_code],
                       marker="D", zorder=2)
